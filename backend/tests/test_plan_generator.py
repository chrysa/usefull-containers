...
class _CompleteStub:
    ...
    def __init__(self, returns: tuple[str | None, ...]):
...
def _stub_complete(monkeypatch, *returns: str | None) -> _CompleteStub:
    stub = _CompleteStub(returns)
    monkeypatch.setattr(plan_generator.ai_client, "complete", stub)
    return stub
...
class TestLlmExtraction:
    async def test_valid_json_is_parsed_and_grounded(self, items, monkeypatch):
        _stub_complete(
            monkeypatch,
    ...
    async def test_code_fenced_json_is_parsed(self, items, monkeypatch):
        _stub_complete(
            monkeypatch,
    ...
    async def test_unknown_item_ids_are_dropped(self, items, monkeypatch):
        _stub_complete(
            monkeypatch,
    ...
    async def test_clarification_is_returned(self, items, monkeypatch):
        _stub_complete(
            monkeypatch,
    ...
    async def test_malformed_output_triggers_one_retry(self, items, monkeypatch):
        stub = _stub_complete(
            monkeypatch,
...
class TestOfflineFallback:
    async def test_regex_fallback_when_llm_disabled(self, items, monkeypatch):
    ...
    async def test_offline_no_match_asks_for_clarification(self, items, monkeypatch):
...
class TestGuards:
    async def test_empty_catalog_asks_to_import_data(self, monkeypatch):
        _stub_complete(monkeypatch)
...
class TestGeneratePlan:
    async def test_single_target_assembles_steps_and_raw(self, items, recipes, monkeypatch):
        _stub_complete(monkeypatch)  # offline path
        resp = await plan_generator.generate_plan(
            "120 iron plate per minute", items, recipes
        )
    ...
    async def test_shared_intermediate_is_merged_once(self, items, recipes, monkeypatch):
    ...
    async def test_vague_prompt_returns_clarification_not_plan(self, items, recipes, monkeypatch):
        _stub_complete(monkeypatch)
